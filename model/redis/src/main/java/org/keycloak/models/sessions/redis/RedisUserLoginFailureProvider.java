...
public class RedisUserLoginFailureProvider implements UserLoginFailureProvider {
    ...
    static final String F_NUM_SECONDARY_AUTH_FAILURES = "numSecondaryAuthFailures";
    ...
    public UserLoginFailureModel addUserLoginFailure(RealmModel realm, String userId) {
        ...
        initial.put(F_NUM_SECONDARY_AUTH_FAILURES, b("0"));
    ...
    private final class RedisUserLoginFailureModel implements UserLoginFailureModel {
        ...
        @Override public int getNumSecondaryAuthFailures() {
            return Integer.parseInt(s(snapshot, F_NUM_SECONDARY_AUTH_FAILURES, "0"));
        }
        @Override public void incrementSecondaryAuthFailures() {
            String v = String.valueOf(getNumSecondaryAuthFailures() + 1);
            snapshot.put(F_NUM_SECONDARY_AUTH_FAILURES, b(v));
            persistField(F_NUM_SECONDARY_AUTH_FAILURES, v);
        }
        @Override public void clearPrimaryAndSecondaryAuthFailures() {
            // Reset secondary in the snapshot first, then clearFailures() HSETs the
            // full snapshot (including secondary=0) atomically.
            snapshot.put(F_NUM_SECONDARY_AUTH_FAILURES, b("0"));
            clearFailures();
        }
