...
import static org.keycloak.models.cache.redis.RedisCacheRealmProviderFactory.REALM_CLEAR_CACHE_EVENTS;
...
public class RedisCacheStoreFactoryProviderFactory implements CachedStoreProviderFactory, EnvironmentDependentProviderFactory {
    ...
    private void lazyInit(KeycloakSession session) {
        if (storeCache == null) {
            synchronized (this) {
                if (storeCache == null) {
                    RedisCache<String, Revisioned> cache = session.getProvider(RedisConnectionProvider.class).getCache(RedisConnectionProvider.AUTHORIZATION_CACHE_NAME);
                    RedisCache<String, Long> revisions = session.getProvider(RedisConnectionProvider.class).getCache(RedisConnectionProvider.AUTHORIZATION_REVISIONS_CACHE_NAME);
