...
public class RedisUserCacheProviderFactory implements UserCacheProviderFactory, EnvironmentDependentProviderFactory {
    ...
    private void lazyInit(KeycloakSession session) {
        if (userCache == null) {
            synchronized (this) {
                if (userCache == null) {
                    RedisCache<String, Revisioned> cache = session.getProvider(RedisConnectionProvider.class).getCache(RedisConnectionProvider.USER_CACHE_NAME);
                    RedisCache<String, Long> revisions = session.getProvider(RedisConnectionProvider.class).getCache(RedisConnectionProvider.USER_REVISIONS_CACHE_NAME);
