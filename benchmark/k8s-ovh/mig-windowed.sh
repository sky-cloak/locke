...
kubectl apply -f benchmark/k8s-ovh/mig-vanilla.yaml >/dev/null 2>&1
...
for w in $(seq 1 16); do
  if [ $w -eq 3 ]; then echo ">>> [$(date -u +%H:%M:%SZ)] APPLY mig-locke.yaml (Infinispan -> Redis)"; kubectl apply -f benchmark/k8s-ovh/mig-locke.yaml >/dev/null 2>&1; flip=1; fi
